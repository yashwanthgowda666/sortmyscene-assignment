...
1. `POST /api/bookings` checks `expiresAt` before confirming — if expired, the seats are released back to `available` and the reservation is deleted, and the request returns a 410 (Gone).
